      ...
      if (resourceId) {
        ...
        setTimeout(() => {
          window.dispatchEvent(
            new CustomEvent("OPEN_CHAT_ROOM", { detail: resourceId })
          );
  ...
  useEffect(() => {
    const fetchUserSession = async () => {
      try {
        ...
        // Remueve el token si la sesión expiró, para no lograr bucle infinito
        localStorage.removeItem("token");
    ...
    <SocketProvider>
      <ChatProvider user={user}>
        <div
          ...
          onClick={() => isNotificationOpen && setIsNotificationOpen(false)}
        >
          <Navbar
            user={user}
