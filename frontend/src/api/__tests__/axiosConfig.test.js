...
describe("Configuración de Axios (axiosConfig)", () => {
  ...
  describe("Entorno ANDROID (Adapter Personalizado)", () => {
    ...
    test("Interceptor de Respuesta: Debe borrar el token si recibe un error 401", async () => {
      // Preparamos el escenario: ponemos un token vencido
      mockLocalStorage.setItem("token", "token-vencido");

      // Obtenemos el cliente
      const client = getApiClient();

      // Simulamos que el servidor rechaza la petición con 401
      ...
      // Hacemos la petición esperando que falle
      try {
        await client.get("/protegido");
      } catch (error) {
        // Esperamos el error, es normal
      }

      // Verificación, ¿Se borró el token?
